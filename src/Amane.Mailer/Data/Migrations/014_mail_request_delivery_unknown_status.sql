-- Expand mail_requests.status CHECK to include DeliveryUnknown (6) (ADR 0022 D-08 / #533).
-- SQLite cannot ALTER CHECK constraints; recreate the table in-place, following the same
-- idiom as 007_mail_request_cancelled_status.sql. PRAGMA foreign_keys=OFF is ineffective
-- inside SqlMigrationRunner's transaction, so back up mail_attempts before DROP to avoid
-- ON DELETE CASCADE data loss.

CREATE TABLE mail_attempts_014_backup AS SELECT * FROM mail_attempts;
...
SELECT
    ...
    accepted_at, created_at, updated_at, completed_at, scheduled_at
FROM mail_requests;
...
INSERT INTO mail_attempts SELECT * FROM mail_attempts_014_backup;

DROP TABLE mail_attempts_014_backup;
