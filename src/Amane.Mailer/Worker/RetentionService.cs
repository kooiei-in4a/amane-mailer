    ...
    ILogger<RetentionService> logger) : BackgroundService
{
    ...
    private async Task PurgeExpiredAsync(CancellationToken cancellationToken)
    {
        ...
        var totalQueueDeadLettersDeleted = 0;
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            var deleted = await providerQueueDeadLetterRepository.DeleteExpiredAsync(
                cutoff,
                retentionOptions.BatchSize,
                cancellationToken);
            ...
            totalQueueDeadLettersDeleted += deleted;
        }
        ...
        if (totalQueueDeadLettersDeleted > 0)
        {
            logger.LogInformation(
                "Retention purge removed {DeletedCount} provider_queue_dead_letters rows older than {RetentionDays} days.",
                totalQueueDeadLettersDeleted,
                retentionOptions.RetentionDays);
        }
