...
internal static class SetupNonInteractiveTestSupport
{
    ...
    /// <summary>
    /// Always nests under a unique child of the system temp directory so Linux fixtures never
    /// attempt to chmod <c>/tmp</c> itself.
    /// </summary>
    internal static string CreateUniqueConfigPath(string fileName = "config.json")
    {
        var root = Path.Combine(Path.GetTempPath(), "amane-ni-" + Guid.NewGuid().ToString("N"));
    ...
    internal static string WriteOwnerOnlyConfig(ISetupFileSystem fileSystem, string absolutePath, string json)
    {
        ...
        if (string.Equals(normalizedDirectory, tempRoot, StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "Config fixtures must use a unique child directory under the system temp path.");
        }
    ...
    internal static string WriteOwnerOnlyConfigOnHost(string json) =>
        WriteOwnerOnlyConfigOnHost(CreateUniqueConfigPath(), json);

    internal static string WriteOwnerOnlyConfigInMemory(
        MemorySetupFileSystem fileSystem,
        string absolutePath,
        string json) =>
