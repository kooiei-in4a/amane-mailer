...
v0.4.0 publish 後に、GHCR ランタイムイメージ（既定 `ghcr.io/kooiei-in4a/amane-mailer:v0.4.0`）を
...
| `MAILER_IMAGE_TAG` | `v0.4.0` | 検証するタグ |
...
`v0.4.0` の value-free smoke 結果（digest、日付、環境、各 check の pass/fail）は
[docs/releases/v0.4.0.md](../releases/v0.4.0.md) に記録します。過去の `v0.3.0` 結果は
