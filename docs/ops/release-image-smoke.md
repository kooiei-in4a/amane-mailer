...
v1.1.0 publish 後に、GHCR ランタイムイメージ（既定 `ghcr.io/kooiei-in4a/amane-mailer:v1.1.0`）を
...
| `MAILER_IMAGE_TAG` | `v1.1.0` | 検証するタグ |
...
`v1.1.0` の value-free smoke 結果（digest、日付、環境、各 check の pass/fail）は
[docs/releases/v1.1.0.md](../releases/v1.1.0.md) に記録します。過去の `v1.0.1` 結果は
