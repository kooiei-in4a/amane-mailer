...
v1.3.5 publish 後に、GHCR ランタイムイメージ（既定 `ghcr.io/kooiei-in4a/amane-mailer:v1.3.5`）を
...
- v1.3.5 release の既定 smoke tag は `v1.3.5`、platform は
...
| `MAILER_IMAGE_TAG` | `v1.3.5` | 検証するタグ |
...
`v1.3.5` の value-free smoke 結果（digest、日付、環境、各 check の pass/fail）は
[docs/releases/v1.3.5.md](../releases/v1.3.5.md) に記録します。過去の `v1.2.0` 結果は
