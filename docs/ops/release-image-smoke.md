...
v1.0.0 publish 後に、GHCR ランタイムイメージ（既定 `ghcr.io/kooiei-in4a/amane-mailer:v1.0.0`）を
...
- v1.0.0 release では、既定 smoke tag `v1.0.0` の GHCR runtime image は publish 後
...
| `MAILER_IMAGE_TAG` | `v1.0.0` | 検証するタグ |
...
`v1.0.0` の value-free smoke 結果（digest、日付、環境、各 check の pass/fail）は
[docs/releases/v1.0.0.md](../releases/v1.0.0.md) に記録します。過去の `v0.9.2` 結果は
