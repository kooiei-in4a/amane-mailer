...
login throttle and session store are in-memory only (no durable store, resets on process restart);
immediate session revocation on admin disable or credential change is not implemented;
