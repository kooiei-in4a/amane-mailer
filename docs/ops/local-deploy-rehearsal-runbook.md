...
audit log は body view と auth イベントを `admin_audit_events` に永続化（stdout にもミラー）。retention sweep は `MAILER_ADMIN_AUDIT_RETENTION_DAYS`（既定 180 日）で自動削除。明示 purge は `db admin-audit purge --older-than-days <days>`。
