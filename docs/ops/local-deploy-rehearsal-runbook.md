...
login throttle と session store は in-memory のみ（durable store なし、プロセス再起動でリセット）、
管理者無効化・認証情報変更時の即時 session 失効は未実装、管理者ごとの tenant scope なし、
audit log は structured log（stdout）のみ（SQLite 永続化は [#6](https://github.com/kooiei-in4a/amane-mailer/issues/6)）。
