...
After v0.9.0 is published, this runbook pulls the GHCR runtime image (default
`ghcr.io/kooiei-in4a/amane-mailer:v0.9.0`) from a clean state, starts Mailer +
...
| `MAILER_IMAGE_TAG` | `v0.9.0` | Tag under test |
...
Value-free smoke results for `v0.9.0` (digest, date, environment, per-check pass/fail)
are recorded in [docs/releases/v0.9.0.md](../releases/v0.9.0.md) after publish.
