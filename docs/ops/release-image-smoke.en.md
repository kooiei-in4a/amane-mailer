...
This runbook pulls the published GHCR runtime image (current public example: `ghcr.io/kooiei-in4a/amane-mailer:v2.3.1`) from a clean state, starts Mailer +
...
exercises the **published release image itself**. Tenant configuration is the safe example baked
...
- For v2.1.0, the runtime image is **`linux/amd64` only**. Confirm the platform in the release notes or Docker
...
```bash
MAILER_IMAGE_TAG=v2.3.1 bash scripts/release-smoke.sh
```
...
Value-free smoke results for `v2.3.1` (digest, date, environment, per-check pass/fail)
are recorded in [docs/releases/v2.3.1.md](../releases/v2.3.1.md).
