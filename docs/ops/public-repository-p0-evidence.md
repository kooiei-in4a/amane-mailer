...
| GHCR image publish | `.github/workflows/promote-qualified-oci.yml` uses `GITHUB_TOKEN` with `contents: read`, `actions: read`, and `packages: write`; no repository secret is needed. The legacy `publish-image.yml` is fail-closed. | Not applicable for repository-stored publish credentials; promotion uses the job-scoped `GITHUB_TOKEN`. | Confirm deploy hosts use only intended read-only pull credentials when GHCR authentication is required. |
