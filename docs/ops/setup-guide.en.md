...
| [v2.1.0 release record](../releases/v2.1.0.md) | Current public release identities / digest / platform / smoke evidence |
...
| **Manual Deployment** | You prefer existing runbooks / CLI without Managed bundles | Modes 1–5 remain available. Current published image is **v2.1.0** (historical release records remain available) |
...
For published **v2.1.0**, use GitHub Release checksums / the [release record](../releases/v2.1.0.md) / the public image digest (<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v2.1.0>). Do not confuse candidate handoff with published release verification. Follow the platform recorded for the release (currently `linux/amd64`) and do not infer unpublished host assets.
...
| Consumer bounced Webhook [#307](https://github.com/kooiei-in4a/amane-mailer/issues/307) | Not in current v2.1.0; future candidate only, with no release promise |
...
For the current release, use [docs/releases/v2.1.0.md](../releases/v2.1.0.md) and [`release/current-public.json`](../../release/current-public.json).
...
Manual Deployment remains a first-class path. The sections below keep the mode 1–5 runbook order and availability meanings. **The current recommended published image is v2.1.0.** Feature-boundary notes that originated in v1.1.0 (for example bounce Queue adoption) remain as historical facts.
...
| [release-image-smoke](release-image-smoke.en.md) | Published-image smoke | For published tags; current example is `v2.1.0` |
...
### About the published image (current v2.1.0)

**Current recommendation:** public GitHub release / GHCR tag `v2.1.0` for both Easy Setup and Manual paths. The source of truth for the current public version, tag, and platform is [`release/current-public.json`](../../release/current-public.json).
Evidence: [docs/releases/v2.1.0.md](../releases/v2.1.0.md) (including release-image smoke) and
<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v2.1.0>. The current runtime image is `linux/amd64` only.
...
[release-image-smoke](release-image-smoke.en.md) uses `v2.1.0` as its current example; always
...
| Published v2.1.0 image not verified | (depends on mode) | See the [v2.1.0 release record](../releases/v2.1.0.md). Hosts not yet on that image: `[WARN]` / `[ACTION]` |
...
7. Verification: `/healthz` `/readyz`, and explicit live send with an approved sender. Published-image smoke: [release-image-smoke](release-image-smoke.en.md) (current example `v2.1.0`; evidence in the [v2.1.0 release record](../releases/v2.1.0.md))
8. If bounce ingestion is needed, continue to mode 5 (otherwise you may stop here)

**Done when:** deploy shape, tenant / env preflight, `Production`-confirmed secret registration, post-registration doctor `platform_sender_environment` PASS, health/ready, and approved live send can be `[PASS]`. Published image is `v2.1.0` ([release record](../releases/v2.1.0.md)).
...
7. Verification: `/healthz` `/readyz`, approved live send. Staging Delivery Report arrival is `setup verify-delivery-report` ([#428](https://github.com/kooiei-in4a/amane-mailer/issues/428)) — not production evidence. Published image is `v2.1.0` ([release record](../releases/v2.1.0.md))
...
**Done when:** mode 4 completion plus compose-wired `queue` settings, Queue file secret, Queue name, and Event Grid → Queue configuration checks can be `[PASS]` / human-confirmed. Published image is `v2.1.0` ([release record](../releases/v2.1.0.md)).
...
- Documenting the Consumer bounce API / webhook contract not included in current v2.1.0 (#307 is a future candidate with no release promise)
