...
| [implementation-status](../implementation-status.json) | Tracked feature status (Easy Setup was introduced in v1.2.0 and remains `implemented`) |
| [v1.3.8 release record](../releases/v1.3.8.md) | Current public release identities / digest / platform / smoke evidence |
...
| **Manual Deployment** | You prefer existing runbooks / CLI without Managed bundles | Modes 1–5 remain available. Current published image is **v1.3.8** (historical release records remain available) |
...
Easy Setup wraps existing `.env` / `tenants.json` / file-secret / deploy compose contracts with a host-local Web or terminal assistant ([ADR 0021](../adr/0021-easy-setup-boundaries.md)). It was **introduced and marked `implemented` in v1.2.0** ([#445](https://github.com/kooiei-in4a/amane-mailer/issues/445) / [#458](https://github.com/kooiei-in4a/amane-mailer/issues/458)); that is feature history, not the current public release version. Use Manual paths when you prefer not to activate Managed bundles.
...
For published **v1.3.8**, use GitHub Release checksums / the [release record](../releases/v1.3.8.md) / the public image digest (<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v1.3.8>). Do not confuse candidate handoff with published release verification. Follow the platform recorded for the release (currently `linux/amd64`) and do not infer unpublished host assets.
...
- Do not accept plaintext passwords via file, redirected stdin, or CLI arguments. Password hash file input is outside the current setup contract.
...
| Consumer bounced Webhook [#307](https://github.com/kooiei-in4a/amane-mailer/issues/307) | Not in current v1.3.8; future candidate only, with no release promise |
...
**Historical note — v1.1.0 → v1.2.0 DB migrations (INCLUDE):** take a backup first; the then-current runtime applied (omission / `none` was not allowed). This is a historical release record, not a current v1.3.8 setup prerequisite:

- `012_provider_event_inbox_details.sql`
- `013_provider_queue_dead_letters.sql`

Historical release identities: [docs/releases/v1.2.0.md](../releases/v1.2.0.md).
For the current release, use [docs/releases/v1.3.8.md](../releases/v1.3.8.md) and [`release/current-public.json`](../../release/current-public.json).
...
Manual Deployment remains a first-class path. The sections below keep the mode 1–5 runbook order and availability meanings. **The current recommended published image is v1.3.8.** Feature-boundary notes that originated in v1.1.0 (for example bounce Queue adoption) remain as historical facts.
...
| [release-image-smoke](release-image-smoke.en.md) | Published-image smoke | For published tags; current example is `v1.3.8` |
...
### About the published image (current v1.3.8)

**Current recommendation:** public GitHub release / GHCR tag `v1.3.8` for both Easy Setup and Manual paths. The source of truth for the current public version, tag, and platform is [`release/current-public.json`](../../release/current-public.json).
Evidence: [docs/releases/v1.3.8.md](../releases/v1.3.8.md) (including release-image smoke) and
<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v1.3.8>. The current runtime image is `linux/amd64` only.

Upgrading an existing deployment is a separate operation from setup. Take a DB backup before
pulling an image or applying migrations, and consult the target release record and normal
migration procedure. The historical `v1.1.0 → v1.2.0` migration remains documented in the
historical note above and the [v1.2.0 release record](../releases/v1.2.0.md).

**Prior public release:** `v1.3.5` evidence remains in
[docs/releases/v1.3.5.md](../releases/v1.3.5.md). Older release migration and feature-boundary
records remain in their respective release records.
If you follow procedures with a local build or develop-derived artifact, record that in your ops notes.

[release-image-smoke](release-image-smoke.en.md) uses `v1.3.8` as its current example; always
provide the verification tag explicitly.
...
| Published v1.3.8 image not verified | (depends on mode) | See the [v1.3.8 release record](../releases/v1.3.8.md). Hosts not yet on that image: `[WARN]` / `[ACTION]` |
...
- [ ] Configuration mode (exactly one from the table). For modes 4 / 5, acknowledge production-specific safety boundaries (dedicated tokens / ACS·Queue isolation, no Push). Treat published image `v1.3.8` as canonical ([release record](../releases/v1.3.8.md)); check [`release/current-public.json`](../../release/current-public.json) when the version or tag changes
...
7. Verification: `/healthz` `/readyz`, and explicit live send with an approved sender. Published-image smoke: [release-image-smoke](release-image-smoke.en.md) (current example `v1.3.8`; evidence in the [v1.3.8 release record](../releases/v1.3.8.md))
8. If bounce ingestion is needed, continue to mode 5 (otherwise you may stop here)

**Done when:** deploy shape, tenant / env preflight, `Production`-confirmed secret registration, post-registration doctor `platform_sender_environment` PASS, health/ready, and approved live send can be `[PASS]`. Published image is `v1.3.8` ([release record](../releases/v1.3.8.md)).
...
7. Verification: `/healthz` `/readyz`, approved live send. Staging Delivery Report arrival is `setup verify-delivery-report` ([#428](https://github.com/kooiei-in4a/amane-mailer/issues/428)) — not production evidence. Published image is `v1.3.8` ([release record](../releases/v1.3.8.md))
...
**Done when:** mode 4 completion plus compose-wired `queue` settings, Queue file secret, Queue name, and Event Grid → Queue configuration checks can be `[PASS]` / human-confirmed. Published image is `v1.3.8` ([release record](../releases/v1.3.8.md)).
...
- Documenting the Consumer bounce API / webhook contract not included in current v1.3.8 (#307 is a future candidate with no release promise)
