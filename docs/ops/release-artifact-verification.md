...
release ごとに、symbols の状態は次の 4 つを分けて記録します。

| 項目 | 確認方法 | release record に記録すること |
| --- | --- | --- |
| 生成 | publish workflow の `Verify symbol package was produced` と summary の `.snupkg` file 名を確認 | `.snupkg` file 名、生成 step の結果 |
| push | `Push symbols to nuget.org` step の結果を確認。`.nupkg` push で symbol package が作成され、明示的な `.snupkg` push が `--skip-duplicate` になる場合もある | push 成功、skip-duplicate、または失敗 |
| availability | NuGet Gallery symbol package endpoint から `.snupkg` を取得し、PDB を含むことを確認 | endpoint、HTTP 結果、file size、PDB path |
| indexing / debugging | NuGet Package Explorer または Visual Studio / Rider debugger で SourceLink / symbol 解決を確認 | verified / not verified と確認方法 |

availability は次のように確認します。
...
`https://api.nuget.org/v3-flatcontainer/.../*.snupkg` は `.nupkg` の flat-container URL と
同じ形でも 404 を返す場合があります。flat-container `.snupkg` 404 だけで symbol package 不在とは
判定せず、NuGet Gallery symbol package endpoint、NuGet Package Explorer、または debugger で確認してください。

NuGet indexing 後は NuGet Package Explorer または Visual Studio / Rider の debugger で SourceLink が
release commit の GitHub source を解決できることを確認します。indexing / debugging を確認できない場合は
`not verified` と記録し、`.snupkg` の生成・push・download availability とは分けて扱います。
