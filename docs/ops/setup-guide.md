...
| [v2.3.0 release record](../releases/v2.3.0.md) | 現行公開 release の identities / digest / platform / smoke 証跡 |
...
| **Manual Deployment** | Managed bundle なしで既存 runbook / CLI を使う | mode 1–5 を維持。現行公開イメージは **v2.3.0**（過去 release の記録は残置） |
...
公開済み **v2.3.0** は GitHub Release の checksum / [release record](../releases/v2.3.0.md) / 公開 image digest を使います（<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v2.3.0>）。candidate handoff と公開リリース検証を混同しないでください。公開 runtime image の platform は release record の記載（現在は `linux/amd64`）に従います。公開 asset の有無や host platform を、記載のない形で補わないでください。
...
| Consumer bounced Webhook [#307](https://github.com/kooiei-in4a/amane-mailer/issues/307) | 現行 v2.3.0 に含まれない。将来候補であり、release promise ではない |
...
現行 release は [docs/releases/v2.3.0.md](../releases/v2.3.0.md) と [`release/current-public.json`](../../release/current-public.json) を参照してください。
...
Manual Deployment は第一級の経路のままです。以下は mode 1–5 の runbook 順と完遂可否の意味を維持します。**現行の推奨公開イメージは v2.3.0** です。bounce Queue 採用など v1.1.0 由来の機能境界の説明は歴史的事実として残します。
...
| [release-image-smoke](release-image-smoke.md) | 公開イメージ smoke | 公開済みタグ向け。現行例は `v2.3.0` |
...
### 公開イメージについて（現行 v2.3.0）

**現行推奨:** 公開 GitHub release / GHCR タグ `v2.3.0`。Easy Setup と Manual の両方でこのタグを正とします。current public version / tag / platform の source of truth は [`release/current-public.json`](../../release/current-public.json) です。
証跡は [docs/releases/v2.3.0.md](../releases/v2.3.0.md)（release-image smoke 含む）および
<https://github.com/kooiei-in4a/amane-mailer/releases/tag/v2.3.0> を参照してください。現行 runtime image は `linux/amd64` のみです。
...
[release-image-smoke](release-image-smoke.md) の現行例は公開済み release（`v2.3.0`）向けです。検証対象 tag は必ず明示してください。
...
| 公開 v2.3.0 イメージ未検証 | （モードによる） | [v2.3.0 release record](../releases/v2.3.0.md) を参照。未追従ホストは `[WARN]` / `[ACTION]` |
...
- [ ] 使う構成モード（上表の 1 つ）。mode 4 / 5 は production 固有の安全境界（専用 token / ACS・Queue 分離、Push 非採用）を理解したうえでの選択。公開イメージは `v2.3.0` を正とする（[release record](../releases/v2.3.0.md)）。version / tag の更新時は [`release/current-public.json`](../../release/current-public.json) を確認する
...
7. Verification: `/healthz` `/readyz`、承認済み sender での明示 live send。公開 release イメージ smoke は [release-image-smoke](release-image-smoke.md)（現行例 `v2.3.0`。証跡は [v2.3.0 release record](../releases/v2.3.0.md)）
8. bounce 取り込みが必要なら mode 5 へ進む（不要ならここで完了してよい）

**完了の目安:** deploy 形・tenant / env preflight・`Production` 確認付き secret 登録・doctor 再実行での `platform_sender_environment` PASS・health/ready・承認済み live send を `[PASS]` にし得る。公開イメージは `v2.3.0`（[release record](../releases/v2.3.0.md)）。
...
7. Verification: `/healthz` `/readyz`、承認済み live send。Staging での Delivery Report 到着確認は `setup verify-delivery-report`（[#428](https://github.com/kooiei-in4a/amane-mailer/issues/428)）— production 実行済みの証拠にはしない。公開イメージは `v2.3.0`（[release record](../releases/v2.3.0.md)）
...
**完了の目安:** mode 4 の完了条件に加え、compose 経由の `queue` 設定・Queue file secret・Queue 名・Event Grid → Queue の構成確認を `[PASS]` / 人手確認できること。公開イメージは `v2.3.0`（[release record](../releases/v2.3.0.md)）。
...
- 現行 v2.3.0 に含まれない Consumer bounce API / webhook 契約（#307 は将来候補。release promise ではない）
