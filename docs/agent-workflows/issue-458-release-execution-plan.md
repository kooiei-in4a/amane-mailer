...
| **Base branch** | `develop` |
| **Plan revision** | **5** / **2026-08-01** |
| **Supersedes** | **Rev.4** / 2026-08-01 (and Rev.3 / Rev.2 / Rev.1) |
| **Encoding** | UTF-8 (no BOM); prefer ASCII arrows (`->`) over rare Unicode punctuation |
| **This round** | Plan document revision only. **Does not execute** version prep, promote, tag, publish, public smoke, sync, commit, push, Issue/PR mutation. **Step 0.0 COMPLETE on develop for Rev.4 plan text** (`issue458PlanCommitSha` / `baseDevelopSha` = `3f2b640c08294502a6796c2634de5fdf03ce776f`). **This Rev.5 plan text is not yet durable** until Agent B APPROVE + new plan-only durability. Phase 1 remains **forbidden**. |
...
## 0. Rev.5 changelog（Agent B B-01 REVISE 対応）

| Finding | Severity | Plan change in Rev.5 |
|---------|----------|----------------------|
| **B-01** | Blocker | Break **B-MIG clearance vs Phase 1 circularity**. Rev.4 required full B-MIG (all nine INCLUDE conditions) clear before Phase 1, but INCLUDE condition 8 needs `releaseCommitSha` from version-prep merge, and condition 9 needs a new binding/run after pin/candidate — a circular gate. **Normative phase-aware split** (align Issue #458 body): **B-MIG-SCOPE** (conds 1-7 + frozen filename list + no-extra-migration policy) -> required before Phase 1 version prep; **B-MIG-PIN** (cond 8: exact inventory on `releaseCommitSha`) -> required after version-prep merge, before #455 / Step 1b.2; **B-MIG-BIND** (cond 9: new binding/run) -> required before #456 qualification start. INCLUDE still needs all nine conditions overall; they clear **in phases**, not as one pre-Phase-1 mega-gate. Update §5, §6.1, §7.1, Steps 0.2 / 0.4 / 1.4 / 1.6 / 1b.0 / 2.1, dependency shape, Next steps. Sister alignment: #456 plan Rev.10 same SCOPE/PIN/BIND split. |
| **B-02 note** (Issue AC; already fixed on GitHub) | Blocker (Issue) | Issue #458 AC Conditional-exception language for migration Hard rows **removed**. Migration Hard (G456-42..44) = **active PASS only**; Conditional exceptions apply only where binding `gateClass=Conditional`. Plan §10 / consumer mapping already treat Hard as active PASS; retain that honesty. |
...
**Step 0.0 status (honest):** COMPLETE on `develop` for the **Rev.4** durable plan file (`issue458PlanCommitSha`=`baseDevelopSha`=`3f2b640c08294502a6796c2634de5fdf03ce776f`). Do **not** re-open Step 0.0 as future-only without acknowledging COMPLETE. **This Rev.5 text supersedes Rev.4 content and is not yet durable** — after Agent B APPROVE of Rev.5, a **new** plan-only durability (Step 0.0 re-run for Rev.5) is required before treating this revision as binding. Do **not** claim Phase 0 complete.

**Rev.4 strengths retained (do not regress):** B-DISPATCH / RC ref; Step 0.0 durability pattern; attempt unity (attempt==1 only); CV-ACTIVE/SCENARIO; merge-commit-only promotion; Option A single #455 workflow run; B-PUB/B-OCI-HANDOFF before 1b.2/#456; exact promotion head == `releaseCommitSha`; D-SEQ; D-ATTEST 方式2 default (now decided EXTERNAL_PROVENANCE); CV-* sealed integrity; B-VAL; completion PR on main after smoke; no UNDECIDED success outputs; Hard sole authority = Issue #456 table; archive manifest immutability; P-OCI-PROMOTE; B-MIG INCLUDE nine conditions (**now phase-aware**).
...
| #456 plan path | `docs/agent-workflows/issue-456-release-qualification-plan.md` (Rev.8 durable on develop; sister **Rev.10** phase-aware B-MIG in progress) | Fact |
...
| **Plan authority for execution of #456** | `docs/agent-workflows/issue-456-release-qualification-plan.md` — sister **Rev.10** (phase-aware B-MIG-SCOPE/PIN/BIND; supersedes Rev.9 draft / Rev.8 durable on develop). Re-pin after sister APPROVE + durability. |
...
| #458 | This issue (publish) | **open** | Plan-only this round (Rev.5 authorship; Phase 1 forbidden) |

Dependency shape (Rev.5 — **non-circular**; phase-aware B-MIG; single #455 run; RC ref + attempt==1):
...
#446 -> #447/#448/#449 -> #450 -> #451 & #459 -> #452/#453/#454
                                    ...
                                    #455 tooling ready; #457 closed
                                              |
Agent B APPROVE of this plan revision (Rev.5)
  -> Step 0.0 plan durability on develop for APPROVED Rev.5 text
     (Rev.4 Step 0.0 already COMPLETE @ 3f2b640...; Rev.5 needs new durability)
  ...
  -> Clear B-MIG-PIN (cond 8: exact inventory on releaseCommitSha)
  ...
  -> Clear B-MIG-BIND (cond 9: new binding/run) before #456 start
...
| **B-MIG-PIN** | Condition 8 incomplete: inventory not proven exact on `releaseCommitSha` | **After version-prep merge; before #455 / Step 1b.2** | Cond 8 needs the pin that Phase 1 creates — cannot clear before Phase 1 | On `releaseCommitSha` tree: migration inventory exact match (filenames + digests); FAIL if extra migrations beyond frozen 012/013 since v1.1.0 tip |
| **B-MIG-BIND** | Condition 9 incomplete: new binding/run not created | **Before #456 qualification start** | Cond 9 needs final Issue/plan/`releaseCommitSha` identities after pin | New binding/run on final Issue body SHA + planCommitSha/planFileSha256 + `releaseCommitSha` |
...
Additional pre-flight (not separate Hard authority): local worktree cleanliness **UNCONFIRMED** at early exploration — re-verify before version-prep PR / promote. Rev.4 Step 0.0 durability is COMPLETE; **Rev.5 plan text** still needs Agent B APPROVE + new Step 0.0 before this revision is binding. Phase 0 is **not** complete while **B-MIG-SCOPE** remains SET.

**Rev.5 order reminder:** Phase 1 start requires **B-MIG-SCOPE cleared** (not full B-MIG). After Step 1.6 pin, require **B-MIG-PIN** before 1b.2/#455. Before Phase 2/#456 start require **B-MIG-BIND**. B-456, B-EVID, and B-GO are **not** blockers for starting Phase 1 version prep. B-RC becomes active after Phase 1 merge (pin). **B-DISPATCH**, **B-PUB/B-OCI-HANDOFF**, and **D-ATTEST** must clear **before** the single #455 run (Step 1b.2). **B-VAL** must clear before Phase 2.3+.
...
### 6.1 B-MIG INCLUDE / EXCLUDE authority（M-R2-03 retained; Rev.5 phase-aware）
...
5. #456 qualification **plan revision** update reflecting migration scope (sister Rev.10 phase-aware split).
6. **Independent review** of that plan revision.
7. Migration qualification **evidence schema** defined and usable.
8. Migration inventory **frozen** to `releaseCommitSha` (exact filenames + digests on that tree).
9. **New binding/run** because Issue body / plan / pin identities change when bodies are amended.

#### Phase-aware clearance (normative; breaks Rev.4 circularity — Agent B B-01)

```text
B-MIG-SCOPE (conds 1-7 + frozen filename list + no-extra-migration policy)
  -> required before Phase 1 version prep
B-MIG-PIN (cond 8: exact inventory on releaseCommitSha)
  -> required after version-prep merge, before #455 / Step 1b.2
B-MIG-BIND (cond 9: new binding/run)
...
14. **Irreversible gates** requiring explicit stop/go: **Step 0.0 plan durability** (Rev.5 re-run after APPROVE), **D-SEQ ACK**, **D-ATTEST ACK**, **B-MIG-SCOPE** (before Phase 1), **B-MIG-PIN** (before 1b.2/#455), **B-MIG-BIND** (before #456), **B-DISPATCH clearance**, **3.5**, **4.2 merge-method check**, **4.4**, **5.0**, **5.1–5.5**, **7.3 merge**, **8.2**.
...
### 7.1 Canonical order (Rev.5)

```text
Agent B APPROVE of Rev.5
  -> Step 0.0 plan durability on develop for Rev.5 text
     (Rev.4 Step 0.0 COMPLETE @ 3f2b640...; do not pretend Rev.5 is already durable)
  ...
  -> Dispatch #455 --ref release/v1.2.0-rc (attempt 1 only)
  -> promote-path proof
  -> Clear B-MIG-BIND (cond 9) -> #456 -> CV-* incl ACTIVE
  -> Phase 4 PR from release/v1.2.0-rc exact head; merge commit only
...
| Owner | Owning Issue |
| This-round status | Always **計画のみ / 未実行** in Rev.5 for Phase 1+; Step 0.0 Rev.4 COMPLETE acknowledged separately |
...
| Preconditions | Agent B **APPROVE** of **this plan revision (Rev.5)**; maintainer authorizes the plan-only durability PR/commit |
| Inputs | Reviewed plan text (this file, Rev.5); `origin/develop` tip |
...
| Stop | Committed text ≠ reviewed text; substantive plan change without re-review; attempting Phase 0 under a non-durable Rev.5 text |
...
| Preconditions | Durable plan for the active revision (Rev.5 Step 0.0 after APPROVE); 0.1 pass |
...
| Inputs | §4.1 D-SEQ; §7.2 Option A; §7.4 D-ATTEST; §7.6 B-DISPATCH; this plan Rev.5 |
...
| Preconditions | Phase 0 scope freeze (**B-MIG-SCOPE cleared**); D-SEQ ACK; worktree OK; durable Rev.5 plan (Step 0.0). Explicit maintainer authorization for Phase 1. **Not** blocked by B-456/B-EVID/B-GO; **not** blocked by B-MIG-PIN/BIND (those clear later) |
...
| Preconditions | 1.1–1.5; **B-MIG-SCOPE cleared**; D-SEQ ACK; worktree clean; durable Rev.5 plan; explicit Phase 1 authorization. Full B-MIG (PIN/BIND) **not** required here |
| Inputs | Touch list; base develop SHA |
| Actions | Branch from frozen develop; open PR; CI green; merge to develop; record **merge result commit** as `releaseCommitSha` / RC `sourceCommitSha`. Immediately after pin, run **B-MIG-PIN** verification (Step 1.6 outputs feed PIN before 1b.2). |
...
| Outputs | `releaseCommitSha` (frozen); clears **B-RC** for subsequent #455/#456 (RC branch still required — Step 1.6.1); enables **B-MIG-PIN** check |
| Owner | #458 |
...
| Actions | (1) Confirm release-infra bootstrap: workflow (+ promote tooling required by B-PUB) exists on default branch `main`; record `infraBootstrapMainSha` (bootstrap may have landed earlier after Step 0.0 — verify still present). (2) Confirm `refs/heads/release/v1.2.0-rc` tip == `releaseCommitSha`. (3) Confirm dispatch will use `--ref release/v1.2.0-rc` (not raw SHA). (4) Confirm attempt-unity policy understood (§7.7). (5) **B-MIG-PIN:** verify migration inventory on `releaseCommitSha` exact match (filenames + digests); FAIL if tree has migrations beyond frozen 012/013 since v1.1.0 tip. |
| Verification | Workflow present on `main`; RC tip equality; no raw-SHA dispatch plan; **B-MIG-PIN cleared** |
| Evidence | `infraBootstrapMainSha`; RC tip SHA; B-DISPATCH clearance note; B-MIG-PIN inventory proof |
| Stop | Workflow missing on `main`; RC tip ≠ `releaseCommitSha`; plan to dispatch @ raw SHA; **B-MIG-PIN FAIL** |
| Re-run | After infra bootstrap PR / RC branch fix / inventory repair (may need new version-prep pin) |
| Outputs | `bDispatchCleared=true`; `infraBootstrapMainSha`; `dispatchRef=release/v1.2.0-rc`; `bMigPinCleared=true` |
| Owner | #458 |
...
| Preconditions | Phase 1c.2 promote-path proof pass; B-RC cleared; attempt-unity recorded; **B-MIG-BIND cleared** (cond 9: new binding/run on final Issue/plan/`releaseCommitSha`) |
| Inputs | Issue #456 body table; plan Rev.10 (sister; phase-aware B-MIG); candidate artifacts; OCI identity; `workflowRunId`; `workflowRunAttempt` |
| Actions | Confirm **B-MIG-BIND** before start. Execute #456 per sister plan (separate authorization) **or** fetch completed sealed package; load binding snapshot; load evidence inventory; load go-no-go decision objects; **verify provenance attempt fields** (`workflowRunAttempt==1`, job attempts == 1, headSha equality, same `workflowRunId`) |
| Verification | Sealed package exists for this SHA/candidateIds; value-free; binds same OCI/archives from `workflowRunId` attempt 1; binding identities match final Issue/plan/pin |
| Evidence | Evidence index digests (not raw logs); attempt-field verification record; B-MIG-BIND clearance |
| Stop | Missing Hard rows; PII in evidence; incomplete inventory; wrong SHA; qualifying a different OCI than 1b.2; attempt>1 / mixed attempts; **B-MIG-BIND still SET** |
| Re-run | New #456 run (after new #455 if candidate identity invalid) |
| Outputs | `qualificationPackageRef`; progresses B-456/B-EVID/B-GO clearance; `bMigBindCleared=true` |
...
| 受入条件（Issue #458） | Phase/Step | 確認方法 | 証拠 | 停止条件 | Rev.5 note |
|------------------------|------------|----------|------|----------|------------|
...
|------|----------|-------------------|
| Circular “#456 before any #458” reading recreates B-01 (Rev.1) | Blocker | D-SEQ ACK; Rev.5 order |
...
**Irreversible gates (must have explicit APPROVE immediately before):** Step 0.0 plan durability (Rev.5 re-run after APPROVE), D-SEQ ACK, D-ATTEST ACK, **B-MIG-SCOPE**, **B-MIG-PIN**, **B-MIG-BIND**, B-DISPATCH clearance, **3.5**, **4.2 merge-method check**, **4.4**, **5.0**, **5.1–5.5**, **7.3**, **8.2**.

---

## 12. 明示的に未実施（Explicitly not done this round）

This planning round (**Rev.5 authorship**) **did not**:

- Step 0.0 durability for **this Rev.5** text (Rev.4 Step 0.0 is COMPLETE @ `3f2b640…`; Rev.5 needs new durability after APPROVE)
...
- main -> develop sync
- commit / push of this Rev.5 authorship
...
## 13. セルフレビュー（1–17）— Rev.5

| # | Check | Result |
|---|-------|--------|
| 1 | Issue #458全受入条件が計画に対応 | **Pass** (§10; INCLUDE AC + active PASS for migration Hard; phase-aware B-MIG) |
...
| 17 | 現在の事実と将来の計画を混同していない | **Pass** (§1; Rev.4 Step 0.0 COMPLETE acknowledged; Rev.5 text not yet durable; Phase 0 not complete; Phase 1 forbidden) |
...
| **M-R2-03** | Major | **Yes** (phase-aware in Rev.5) | §6.1 nine INCLUDE conditions + SCOPE/PIN/BIND; Steps 0.2/1.4/1b.0/2.1 |
...
### 13.3 Agent B B-01 (Rev.5) finding closure table

| Finding | Severity | Closed in Rev.5? | Where |
|---------|----------|------------------|-------|
| **B-01** (B-MIG vs Phase 1 circularity) | Blocker | **Addressed in plan text — needs Agent B re-review** | §0, §5, §6.1, §7.1, Steps 0.2 / 0.4 / 1.4 / 1.6 / 1b.0 / 2.1, Next steps |

**Self-review verdict:** **Pass for authorship completeness of the B-01 fix** (17/17 checks as plan-text). **Not** an execution APPROVE. **Rev.5 needs independent Agent B re-review** (APPROVE / REVISE). Residual: **B-MIG-SCOPE/PIN/BIND SET** (umbrella B-MIG SET); **B-DISPATCH**, **B-PUB/B-OCI-HANDOFF**, **B-VAL**, plus later **B-RC** / **B-456** / **B-EVID** / **B-GO**. Rev.4 Step 0.0 outputs remain valid for the **Rev.4** durable file only; **this Rev.5 plan text is not yet durable** until new plan-only durability after APPROVE. Do **not** claim Phase 0 complete. **Phase 1 still forbidden.**

---

## 14. Next steps + Agent B re-review note

1. Independent **Agent B** re-review (APPROVE / REVISE) against Issue #458 + this **Rev.5** document + #456 Rev.10 (sister B-MIG-SCOPE/PIN/BIND) + ADR 0021 + current `generate-setup-release-candidate.yml`. Agent B must not modify the repo. Focus: **B-01** phase-aware B-MIG split (no pre-Phase-1 mega-gate; SCOPE before Phase 1; PIN before #455; BIND before #456).
2. After Agent B **APPROVE**: execute **Step 0.0 for Rev.5** (plan-only PR/commit of APPROVED Rev.5 text -> merge to develop -> fix new `issue458PlanCommitSha` / `issue458PlanFileSha256` -> refresh `baseDevelopSha`). Rev.4 Step 0.0 COMPLETE @ `3f2b640…` remains historical fact for that file revision. **This authorship round still does not commit.**
...
7. Clear **B-VAL** by landing a version-pinned `#456 consumer validator` implementing `CV-*` including ACTIVE/SCENARIO (or an explicitly accepted enumerated equivalent for sealed-package integrity).
8. Re-verify local worktree clean + fast-forward `develop` to `origin/develop` (tip at execution time).
9. Only after Agent B APPROVE **and** Rev.5 Step 0.0 **and** **B-MIG-SCOPE cleared** + D-SEQ/D-ATTEST recorded + **explicit maintainer authorization**: begin Phase 1 version prep (**B-MIG-PIN/BIND and B-456/B-EVID/B-GO not required to start Phase 1**).
10. After version-prep merge: freeze SHA (**B-RC**), clear **B-MIG-PIN**, create `release/v1.2.0-rc`, clear B-DISPATCH + B-PUB/B-OCI-HANDOFF, single #455 run `--ref release/v1.2.0-rc` (**attempt 1 only**), promote-path proof (1c.2), clear **B-MIG-BIND**, then #456, then #458 consumer validation (B-VAL + CV-ACTIVE), then promote (**merge commit only**) / tag on `releaseCommitSha` / publish.
11. Keep `easy-setup` at `partial` until Phase 6 public smoke + Phase 7 completion PR on main.

**Canonical next-step order:**

```text
Agent B APPROVE (Rev.5)
  -> Step 0.0 durability for Rev.5
  -> Clear B-MIG-SCOPE
...
**No release execution in this round. Phase 1 still forbidden.**
...
| Artifact | Owner path |
|----------|------------|
| Qualification plan | `docs/agent-workflows/issue-456-release-qualification-plan.md` (Rev.10 sister; phase-aware B-MIG) |
| This execution plan | `docs/agent-workflows/issue-458-release-execution-plan.md` (**Rev.5**) |
...
| B-MIG-PIN | Cond 8 exact inventory on releaseCommitSha; after 1.6, before #455 / 1b.2 |
| B-MIG-BIND | Cond 9 new binding/run; before #456 qualification start |
...
## Appendix C — Normative #455 job mapping (Rev.5)
...
#   B-MIG-PIN cleared (cond 8 exact inventory on releaseCommitSha)
...
-> Clear B-MIG-BIND (cond 9) before Phase 2 #456
...
End of Issue #458 Release Execution Plan **Rev.5** (2026-08-01). Supersedes Rev.4, Rev.3, Rev.2, and Rev.1.
