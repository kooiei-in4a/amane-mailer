...
| Manual cancel vs stale `Processing` | reaper と競合しうる。`UPDATE ... WHERE status=Processing AND lock_expires_at <= @Now` で **先勝ち**。負け側は 0 行更新 → 呼び出し元は `invalid_state` または `lock_held` で失敗応答 |
...
### D-05. `attempt_count` / `max_attempts` / `next_attempt_at` の扱い（要約）
...
`mail_attempts.attempt_number` は Worker が記録した配送試行の連番であり、手動再送で `attempt_count` を 0 に戻しても **過去行は削除しない**。
...
| scope 外 | **403 Forbidden**、`error_code=tenant_forbidden`、監査 `failure` |
| 存在しない ID | **404 Not Found**（tenant 漏洩防止のため scope 外と同じ 404 でもよい — #101 で既存 Admin 詳細と揃える） |
...
|--------------|----------|---------------|-------------|----------------------|
| `mail_request.manual_retry_requested` | 手動再送 API 処理時（成功・失敗とも） | `mail_request` | 内部 `mail_requests.id` (UUID) | `result`, `error_code`（例: `invalid_state`, `tenant_forbidden`） |
...
監査書き込みは **best-effort**（[AdminAuditLog.WriteBestEffortAsync](../../src/Amane.Mailer/Admin/AdminAuditLog.cs)）とする。操作本体の成否は DB 更新の成否で決まり、監査失敗だけで再送・キャンセルをロールバックしない（auth 系と同じ）。
...
| 応答 | 成功: **303 See Other** で詳細または一覧へリダイレクト（既存 Admin パターン）。失敗: **409**（invalid state / lock held）、**403**（tenant）、**404** |
