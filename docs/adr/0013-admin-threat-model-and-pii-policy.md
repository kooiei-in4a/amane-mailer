...
本 ADR は後続の Admin UI 方式調査（#174）、Admin API・認証境界・StaticFiles 配信基盤（#175）、一覧画面（#177）、管理操作監査ログ（#183）、DB 操作（#185）の実装前提を凍結する。
...
### D-03. 管理画面の bind address を制限できるようにする

管理画面の待受または公開先は `MAILER_ADMIN_BIND` で制限できるようにする。ホスト上で直接待ち受ける構成の既定値は `127.0.0.1` とする。

実装方式として管理画面専用 listener を分けられる場合は、`MAILER_ADMIN_BIND=127.0.0.1` を直接適用する。Docker 内で管理画面 listener が `0.0.0.0` を使う必要がある場合は、ホスト側の publish address、reverse proxy、Docker network、firewall による到達制限を有効な bind 境界として扱い、その制約を #175 の受け入れ条件に含める。

`0.0.0.0` など外部到達可能な bind は、`MAILER_ADMIN_ENABLED=true` と別に明示設定を必要とし、運用ドキュメントで到達制限を確認してから使う。

### D-04. 認証方式は Cookie + PBKDF2 パスワードを第一候補にする

ブラウザ UX、自己完結性、reverse proxy 依存の低さを優先し、アプリ内の Cookie session + PBKDF2 パスワード認証を採用する。#174 で Native AOT 互換性を検証し、互換性に問題がないことを #175 の実装前提にする。

管理者アカウントは Consumer アプリの `admin_users` とは共有しない。Mailer は自身の SQLite DB に管理者・権限・セッションの正本を持ち、Consumer 側のロール・所有権とは独立して運用する。

最低要件は以下とする。

- パスワード hash は PBKDF2-HMAC-SHA256 以上、salt はユーザーごと、反復回数は設定可能。既定値と production 下限は 600,000 回以上とし、#174 の計測で上げることはできるが下げない
...
### D-11. Phase A/B の管理画面は単一 Mailer プロセスを前提にする

ADR 0012 D-06a と `infra/deploy/compose.yml` の形に合わせ、Phase A/B の管理画面は単一 Native AOT container / 単一 Mailer プロセスで運用する。管理 API や管理画面だけを水平スケールさせる構成は Phase A/B の対象外とする。
...
PBKDF2 の 600,000 回以上という下限は #174 で低リソース環境の CPU 負荷も測る。検証では、ログイン失敗の連打が配送 worker、HTTP readiness、healthcheck に与える影響を確認し、必要ならログイン endpoint の並列数制限を #175 の基盤要件に含める。
...
- _positive:_ #174 は Cookie + PBKDF2 を第一候補として AOT 互換性を検証すればよく、#175 は fail-closed、bind 制限、CSRF、監査 hook を基盤要件にできる。
- _positive:_ #177 は一覧 PII マスクと本文非表示を前提に設計でき、非マスク表示や本文表示は明示操作として扱える。
- _positive:_ #183 は監査対象イベントと保持期間の初期値を本 ADR から引ける。
- _positive:_ #185 は `MAILER_ADMIN_DB_OPS_ENABLED=true` を必須前提にできる。
- _negative:_ 障害対応時に宛先・件名・本文を即時確認できず、1 クリック以上の操作と監査ログが増える。
- _negative:_ 専用 listener または reverse proxy / network ACL による到達制限が必要になり、compose と運用手順の確認項目が増える。
...
- _negative:_ Phase A/B は単一 Mailer プロセス前提のため、水平スケールで管理画面を動かすには session / throttle / DB 操作ロックを再設計する必要がある。
