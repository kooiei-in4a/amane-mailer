...
実装方式として管理画面専用 listener を分けられる場合は、`MAILER_ADMIN_BIND=127.0.0.1` を直接適用する。Docker 内で管理画面 listener が `0.0.0.0` を使う必要がある場合は、ホスト側の publish address、reverse proxy、Docker network、firewall による到達制限を有効な bind 境界として扱い、その制約を管理画面基盤の受け入れ条件に含める。
...
- _positive:_ Cookie + PBKDF2 を第一候補として AOT 互換性を検証すればよく、管理画面基盤は fail-closed、bind 制限、CSRF、監査 hook を基盤要件にできる。
