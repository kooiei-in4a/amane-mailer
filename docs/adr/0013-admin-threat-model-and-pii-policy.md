...
| 監査 auth 残件・retention・hash 化 | D-08 | logout / session expired / login rate limited、retention sweep、network identifier hash 化は未実装 | [ADR 0014 D-04](0014-admin-session-tenant-throttle-audit-design.md#d-04-監査-follow-uplogout--session-expired--rate-limited--retention-sweep) Phase 1（auth イベント）/ Phase 3（保持運用） |
