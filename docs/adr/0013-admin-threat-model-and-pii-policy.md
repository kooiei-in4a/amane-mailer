...
| durable session store・session 即時失効 | D-04 | in-memory cookie auth のみ（無効化・認証情報変更時の即時失効なし） | — |
| 管理者ごとの tenant scope | D-10 | 未実装（単一 `AMANE_ADMIN_USERNAME` / `AMANE_ADMIN_PASSWORD_HASH`） | — |
| 管理操作監査ログの SQLite 永続化 | D-08 | structured log（stdout）のみ | [#6](https://github.com/kooiei-in4a/amane-mailer/issues/6) |
