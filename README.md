...
v2.0.0 publish 後の GHCR イメージ（既定 `ghcr.io/kooiei-in4a/amane-mailer:v2.0.0`）を clean state から
...
公開 identities は [v2.0.0 release record](docs/releases/v2.0.0.md) /
[GitHub Release](https://github.com/kooiei-in4a/amane-mailer/releases/tag/v2.0.0) です。

v2.0.0 release の GHCR runtime image は **`linux/amd64` only** です。
現在公開中の release tag は `v2.0.0` ですが、smoke 実行時は `MAILER_IMAGE_TAG` または
