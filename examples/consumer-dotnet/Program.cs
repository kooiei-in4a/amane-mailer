...
Console.WriteLine($"POST {options.MailerBaseUrl}internal/mail-requests");
Console.WriteLine($"mail_request_id: {mailRequestId}");
Console.WriteLine($"payload_hash:    {request.PayloadHash}");
Console.WriteLine();
...
if (response.StatusCode == HttpStatusCode.Accepted)
{
    ...
    {
        Console.WriteLine("This mail_request_id was already accepted with the same payload_hash;");
...
else if (response.StatusCode == HttpStatusCode.Conflict)
{
    ...
    Console.WriteLine("different payload_hash. Reusing a mail_request_id after changing subject,");
    ...
    bool Mutate)
{
    public static SampleOptions Parse(string[] args)
    {
        ...
        return new SampleOptions(
            ...
            MailServiceToken: Environment.GetEnvironmentVariable("MAIL_SERVICE_TOKEN") ?? "local-mail-service-token",
            TenantId: Guid.Parse(
                Environment.GetEnvironmentVariable("MAILER_TENANT_ID")
                ?? "00000000-0000-0000-0000-000000000101"),
            SourceService: Environment.GetEnvironmentVariable("MAILER_SOURCE_SERVICE") ?? "example-service",
