...
# to/cc/bcc follow the same omission rule (ADR 0023 D-02): unspecified, null, and an empty list
# are all equivalent and the role is omitted from the hash document entirely (no "to":null or
# "to":[] ever appears; a CC-only request has no "to" key at all). A non-empty role is
# re-projected to the validated canonical recipient value (trimmed email; whitespace-only
# display_name treated as absent) -- not the raw request bytes -- so equivalent-but
# differently-formatted submissions hash identically.
...
def _project_recipient_role(
    ...
    """Projects a validated to/cc/bcc role to its hash-document shape, or None to omit it."""
    if not role:
        return None
    projected = []
