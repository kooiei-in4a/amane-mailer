...
|------|------------------|-----------------------------------|---------------------|----------------------|
| Current public release | [`release/current-public.json`](release/current-public.json) for the published version, tag, platform, and release-record path | Governed current-version text in README, setup guides, `SECURITY.md`, `ROADMAP.md`, and release-smoke defaults; the deterministic post-release sync updates its bounded follower set | `node scripts/check-release-smoke-tag-drift.mjs` validates the authority and the complete governed set | Do not hand-edit this authority, change a follower first, or use a historical `docs/releases/` record to select the current release. Follow the coordinated post-release procedure in `docs/agent-workflows/release.md`; do not assume every governed follower is machine-edited. |
...
| Release preparation and evidence metadata | The coordinated release identity and lifecycle defined by [`docs/agent-workflows/release.md`](docs/agent-workflows/release.md): Contracts project version, OpenAPI version, reviewed `CHANGELOG.md` entry, and the matching `docs/releases/vX.Y.Z.md` record | Git tag, GHCR image and labels, NuGet package, GitHub Release, then `release/current-public.json` and its followers after verified publication | Run the phase-appropriate `release.ps1 status`, `preflight`, or `verify` command with explicit `-Version` and, where required, `-ReleaseCommitSha` as documented in the release workflow; review the target `CHANGELOG.md` entry and PENDING/PUBLISHED release record; run restore/build/test, the OpenAPI and current-public checks above, and `powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\release-client-self-test.ps1` | Do not treat the fixture-backed self-test as live-state validation, hand-create publication evidence, advance `current-public` during version preparation, or edit machine-owned release fields outside the guarded `release.ps1` workflow. Release/publish operations are maintainer-only. |
